            ...
            for (Cookie cookie : cookies) {
                if (cookie.getName().equals(GlobalConst.COOKIE_TOKEN)) {
                    ...
                    if (!StringUtils.isEmpty(cookieValue)) {
                        ...
                        if (adminInfo != null && adminInfo.getUsername() == "SuperAdmin") {
...
/*
    ...
    public String list(HttpServletRequest request, ModelMap map) {
        ...
        if (cookies != null) {
            for (Cookie cookie : cookies) {
                if (cookie.getName().equals(GlobalConst.COOKIE_TOKEN)) { // 假设cookie名称是用户名
                    String username = cookie.getValue();
                    log.info("获取到存储的用户名cookie={}", username);

                    if (!StringUtils.isEmpty(username)) {
                        AdminInfo adminInfo = repository.findByUsername(username); // 使用用户名查找AdminInfo

                        if (adminInfo != null && adminInfo.getAdminId() == 1) { // 检查adminId是否为1
                            map.put("isAdmin", true);
                        }
                    }
                }
