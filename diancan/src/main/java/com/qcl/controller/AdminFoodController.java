...
*/
    ...
    public String list(@RequestParam(value = "page", defaultValue = "1") Integer page,
                       @RequestParam(value = "size", defaultValue = "10") Integer size,
                       ModelMap map) {
        PageRequest request = PageRequest.of(page - 1, size); // 创建分页请求
  int i= GlobalData.ADMIN_ID;
        Page<Food> foodPage = foodRepository.findByAdminId(i, request); // 根据adminId和分页请求查找食品

        if (foodPage != null) {
            map.put("foodPage", foodPage); // 将分页的食品列表放入ModelMap
            map.put("currentPage", page); // 放入当前页码
            map.put("size", size); // 放入每页大小
        }

        return "food/list"; // 返回对应的模板名称
    }
            ...
            try {
                for (Food excel : list) {
                    if (excel != null) {
                        // 在这里给admin_id字段赋值为3
                        int i = GlobalData.ADMIN_ID;
                        excel.setAdminId(i); // 假设Food类中有setAdminId这个方法
